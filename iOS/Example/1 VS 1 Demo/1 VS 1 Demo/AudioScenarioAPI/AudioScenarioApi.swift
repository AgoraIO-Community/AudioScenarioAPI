...
public class AudioScenarioApi: NSObject {
    ...
    func initialize() {
        apiRepoter = APIReporter(type: .audioScenario, version: self.version, engine: self.rtcEngine)
    ...
    public func setAudioScenario(sceneType: SceneType, audioScenarioType: AudioScenarioType) {
        scenarioApiLog(msg: "setAudioScenario, sceneType:\(sceneType.rawValue), audioScenarioType:\(audioScenarioType.rawValue)")
        apiRepoter?.reportFuncEvent(name: "setAudioScenario", value: ["sceneType" : sceneType.rawValue, "audioScenarioType" : audioScenarioType.rawValue], ext: [:])
...
extension AudioScenarioApi: AgoraRtcEngineDelegate {
    ...
    private func enableVirtualSoundCard(soundCardType: SoundCardType) {
        apiRepoter?.reportFuncEvent(name: "enableVirtualSoundCard", value: ["soundCardType" : soundCardType.presetValue], ext: [:])
    ...
    private func scenarioApiLog(msg: String) {
        apiRepoter?.writeLog(content: msg, level: .info)
    }
    
    private func scenarioApiLogError(msg: String) {
        apiRepoter?.writeLog(content: msg, level: .error)
    }
}
