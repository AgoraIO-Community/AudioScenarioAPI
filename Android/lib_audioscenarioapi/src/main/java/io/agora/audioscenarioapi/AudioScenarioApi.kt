...
class AudioScenarioApi(rtcEngine: RtcEngine): IRtcEngineEventHandler() {
    ...
    private var apiReporter: APIReporter? = null

    init {
        this.rtcEngine = rtcEngine
        this.apiReporter = APIReporter(APIType.AUDIO_SCENARIO, version, rtcEngine)
        // 注册事件监听
        rtcEngine.addHandler(this)
    }

    fun setAudioScenario(sceneType: SceneType, audioScenarioType: AudioScenarioType) {
        scenarioApiLog("setAudioScenario, sceneType:$sceneType, audioScenarioType:$audioScenarioType")
        apiReporter?.reportFuncEvent("setAudioScenario", mapOf("sceneType" to sceneType.value, "audioScenarioType" to audioScenarioType.value), mapOf())
    ...
    private fun enableVirtualSoundCard(soundCardType: SoundCardType, earPhoneType: EarPhoneType? = null) {
        apiReporter?.reportFuncEvent("enableVirtualSoundCard", mapOf("soundCardType" to soundCardType.name), mapOf())
        // 有预设的耳机类型
        earPhoneType?.let {
    ...
    private fun scenarioApiLogError(msg: String) {
        apiReporter?.writeLog("[$tag] $msg", Constants.LOG_LEVEL_ERROR)
    }
}